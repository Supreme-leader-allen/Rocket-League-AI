...
if ! grep -q 'ANNEAL_SECONDS = float(os.environ' Train_Ground.py; then
    cat >&2 <<'MSG'
BLOCKED: Train_Ground.py hardcodes ANNEAL_SECONDS as a literal. Expose it first:

    ANNEAL_SECONDS = float(os.environ.get("ANNEAL_SECONDS", 2 * 60 * 60))
